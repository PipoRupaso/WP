...
копьё, ночь (факелы/сон), коллизии. Запуск: python3 tests/vtest.py"""
import os
import sys
import math
...
def force_done(v, hid, pos):
    p = M._vil_new_plot(v, hid, pos)
    if p is None:
        return None
    while p["stage"] < len(p["stages"]):
        M._plot_spawn_stage(v, p)
    return p
...
stumps = sum(1 for t in G.VIL_TREES if t["o"].get("dead") == "stump")
...
v3 = G.VIL
M._vil_solids(v3)
h3 = dict(M._vil_make_human(99, fx2 + 1.6, fy2))
h3["state"] = "walk"
h3["act"] = "task"
...
for _ in range(30 * 60):
    M._vil_worker_step(v3, h3, 1 / 60.0)
