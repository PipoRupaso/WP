...
# ---------------------------------------------------------------------------
# Поселение в общей глубинной сортировке мира
# ---------------------------------------------------------------------------
def _night_v():
    return clamp((0.6 - G.LIGHT["amb"]) / 0.35, 0.0, 1.0)


def draw_blood(window, cam, ctx, b):
    bx, by = b["x"], b["y"]
    sx, sy = cam.world_to_screen(bx, by, _vil_gz(bx, by))
    gr = min(1.0, b["t"] * 2.0)
    fade = max(0.3, 1.0 - max(0.0, b["t"] - 45.0) / 45.0)
    rr = max(2, int(b["r"] * 30.0 * ctx[3] * (0.45 + 0.55 * gr)))
    col = (int(118 * fade), int(16 * fade), int(15 * fade))
    pygame.draw.ellipse(window, col,
                        (sx - rr, sy - rr // 3, rr * 2, rr * 2 // 3 + 1))
    if b["big"]:
        for k in range(5):
...
def draw_village_item(window, cam, ctx, kind, payload):
    ze = ctx[3]
    if kind == "stick":
        s = payload
        ...
        pygame.draw.line(window, tone(shade(BASE_COLORS["wood_dark"],
                                            PZ), 0.95),
                         p1, p2, max(1, SP(2) if ze > 0.5 else 1))
    elif kind == "log":
        lg = payload
        ...
        pygame.draw.ellipse(window, cc,
                            (sp["_sx"] - rr, sp["_sy"] - rr // 2,
                             rr * 2, rr))
    elif kind == "fire":
        _vil_draw_fire(window, cam, ctx)


def draw_human_full(window, cam, ctx, hum):
    ze = ctx[3]
    sx, sy = cam.world_to_screen(hum["x"], hum["y"], hum["z"])
    moving = hum["state"] in ("walk", "run", "hunt")
    if moving:
        ...
        hum["ph"] += 1.6 / 60.0
    nightv = _night_v()
    hum["torch_on"] = bool(hum.get("torch")) and nightv > 0.25
    S = max(8, int(round(TILE_Z * ze * 1.1)))  # пиксель арта как у зверей
...
def collect_village_items(cam):
    """Люди, звери, жерди и кровь — в общий список глубинной сортировки."""
    from game.render.depth import T_ACTOR, T_DECAL, T_OBJECT
    from game.render.depth import depth_key
    ctx = _proj_ctx(cam)
    v = G.VIL
    items = []
    for b in v["blood"]:
        sx, sy = cam.world_to_screen(b["x"], b["y"], _vil_gz(b["x"], b["y"]))
        if sx < -40 or sy < -40 or sx > cam.win_w + 40 \
                or sy > cam.win_h + 40:
            continue
        items.append((depth_key(ctx, b["x"], b["y"]), 0.0, T_DECAL,
                      ("blood", b)))
    for s in v["sticks"]:
        ...
        if sx < -40 or sy < -40 or sx > cam.win_w + 40 \
                or sy > cam.win_h + 40:
            continue
        items.append((depth_key(ctx, s["x"], s["y"]), s["z"], T_OBJECT,
                      ("vi", "stick", s)))
    for lg in v["logs"]:
        ...
        if sx < -40 or sy < -40 or sx > cam.win_w + 40 \
                or sy > cam.win_h + 40:
            continue
        items.append((depth_key(ctx, lg["x"], lg["y"]), lg["z"], T_OBJECT,
                      ("vi", "log", lg)))
    for sp in v.get("stones_placed", ()):
        ...
        if sx < -40 or sy < -40 or sx > cam.win_w + 40 \
            ...
            continue
        sp["_sx"], sp["_sy"] = sx, sy
        items.append((depth_key(ctx, sp["x"], sp["y"]), 0.0, T_OBJECT,
                      ("vi", "stonep", sp)))
    for s2 in v.get("pebbles", ()):
        ...
        if sx < -40 or sy < -40 or sx > cam.win_w + 40 \
            ...
            continue
        s2["_sx"], s2["_sy"] = sx, sy
        items.append((depth_key(ctx, s2["x"], s2["y"]), s2["z"], T_OBJECT,
                      ("vi", "pebble", s2)))
    fx, fy = v["fire"]
    items.append((depth_key(ctx, fx, fy), 0.0, T_OBJECT, ("vi", "fire", None)))
    for hum in v["workers"]:
        sx, sy = cam.world_to_screen(hum["x"], hum["y"], hum["z"])
        if sx < -60 or sy < -60 or sx > cam.win_w + 60 \
                or sy > cam.win_h + 60:
            continue
        items.append((depth_key(ctx, hum["x"], hum["y"]), hum["z"],
                      T_ACTOR, ("hum", hum)))
    for an in v["animals"]:
        ...
        if sx < -60 or sy < -60 or sx > cam.win_w + 60 \
                or sy > cam.win_h + 60:
            continue
        items.append((depth_key(ctx, an["x"], an["y"]), an["z"],
                      T_ACTOR, ("animal", an)))
    return items


def draw_village_overlay(window, cam, frame):
    """Аддитивные свечения и текст поражения — поверх кадра."""
    v = G.VIL
    if v is None:
        return
    nightv = _night_v()
    if nightv > 0.2 and v["lit"]:
        ...
        ze = ctx[3]
        fx, fy = v["fire"]
        c0 = cam.world_to_screen(fx, fy, _vil_gz(fx, fy))
        r = max(24, int(TILE_W / 2 * ze * 3.2))
        gl = _radial_glow(("fire", r), r, (255, 150, 50), 46)
        window.blit(gl, (int(c0[0]) - r, int(c0[1]) - r * 0.7))
    if nightv > 0.25:
        for hum in v["workers"]:
            if hum.get("dead") or not hum.get("torch"):
                continue
            sx, sy = cam.world_to_screen(hum["x"], hum["y"], hum["z"])
            if sx < -60 or sy < -60 or sx > cam.win_w + 60 \
                    or sy > cam.win_h + 60:
                continue
            S = max(8, int(round(TILE_Z * ctx[3] * 1.1)))
            fx2 = int(sx + S * 0.17)
            fy2 = int(sy - S * 1.05)
