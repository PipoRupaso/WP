...
Usage: python3 tools/render_showcase.py  (PNG -> screenshots/showcase/)
"""
import os
import sys
...
os.makedirs(os.path.join(HERE, "screenshots", "showcase"), exist_ok=True)
os.chdir(os.path.join(HERE, "screenshots", "showcase"))
...
BIG = (W * G.PIXEL, H * G.PIXEL)  # 1278x720
...
def _smoke_r(e, zoom):
    k = e["t"] / e["life"]
    return int((e["r"] * (M.TILE_W / 2) + 26 * k) * zoom / G.PIXEL)
